...
const totalSum = (input) => {
  return input.flat().reduce((runTot, curVal) => runTot + curVal, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(nestArr => {
    return nestArr.filter(num => num % 5 === 0 && typeof num === 'number').map(filtNum => Math.pow(2, filtNum));
  });
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findShortest = (data) => {
  return data.reduce((shortestATM, nextChar) => +shortestATM.height < +nextChar.height ? shortestATM : nextChar).name;
};
